...
echo -e "\nWelcome to Trenton's Salon. How may I assist you today?\n"
...
MAIN_MENU(){
  ...
  then
  echo "Sorry, that is not available right now"
  else
  echo "$AVAILABLE_SERVICES" | while read SERVICE_ID BAR NAME
    ...
    then
     MAIN_MENU "That is not a number here."
     else
      ...
      if [[ -z $SERVICE_AVAILABLE ]]
        then
         MAIN_MENU "I couldn't find that right now. What would you like today?"
         else
